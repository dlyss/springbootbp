...
public class StringUtil {
    ...
    public static String replaceStr(String str, String field, String value, String re_str) {
        //开始位置
        int begin = str.indexOf(field);
        //从开始位置往后第一个引号位置
        int fh = str.substring(begin, str.length()).indexOf("'");
        //需要替换的字符串 (从 起始位置 到 值之后的引号位置) 后面加的2为 2个引号
        String temp = str.substring(begin, begin + fh + value.length() + 2);
        //替换值
        str = str.replace(temp, re_str);
        return str;
    }
